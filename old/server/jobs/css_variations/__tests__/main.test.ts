...
describe("Job css_variations main", () => {
	beforeEach(() => {
		jest.resetAllMocks();
		DB.getCSSFileVariationByID.mockReturnValue(
			promReturn({
		...
		Download.toVar.mockReturnValue(
			promReturn("h1{color:red;} .cc {border: 2px}")
		);
	});

	test("should do nothing when no new selectors are detected", async () => {
		DB.getAllSelectorsWithFilter.mockReturnValue(
			promReturn([
	...
	test("should detect new detector", async () => {
		...
		return main(logger, CSS_VAR_ID, mockCallback).then(() => {
			...
			expect(insertSelectors).toHaveBeenCalledWith(
				expect.anything(),
				APP_Id,
				m,
				false
	...
	test("should totally delete selectors that are not found in the variation", async () => {
		DB.getAllSelectorsWithFilter.mockReturnValue(
			promReturn([{ id: 55, selector: ".remove", mapping_state: "no" }])
		);
	...
	test("should remove a mapping of selectors that are not found in the variation", async () => {
		DB.getAllSelectorsWithFilter.mockReturnValue(
			promReturn([{ id: 55, selector: ".remove", mapping_state: "has" }])
		);
